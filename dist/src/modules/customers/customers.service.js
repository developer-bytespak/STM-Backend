...
let CustomersService = class CustomersService {
    ...
    async getCustomerDashboard(userId) {
        ...
        const [jobStats, totalSpent, pendingFeedback, recentJobs, recentFeedback] = await Promise.all([
        ...
        return {
            ...
            recentFeedback: recentFeedback.map((feedback) => ({
                ...
                provider: feedback.provider.business_name ||
    ...
    async performJobAction(userId, jobId, dto) {
        ...
        if (dto.action === job_action_dto_1.CustomerJobAction.APPROVE_EDITS) {
            if (!job.pending_approval) {
                throw new common_1.BadRequestException('No pending edits to approve');
            }
            return await this.prisma.$transaction(async (tx) => {
                const editedAnswers = job.edited_answers;
                const updateData = { pending_approval: false };
        ...
        else if (dto.action === job_action_dto_1.CustomerJobAction.CLOSE_DEAL) {
            if (job.status !== 'new') {
                throw new common_1.BadRequestException('Job is not in new status');
            }
            if (!job.sp_accepted) {
                throw new common_1.BadRequestException('Service provider has not accepted yet');
            }
            return await this.prisma.$transaction(async (tx) => {
                await tx.jobs.update({
                    where: { id: jobId },
        ...
        else if (dto.action === job_action_dto_1.CustomerJobAction.CANCEL) {
            if (!['new', 'in_progress'].includes(job.status)) {
                throw new common_1.BadRequestException('Cannot cancel this job');
            }
            if (!dto.cancellationReason) {
                throw new common_1.BadRequestException('Cancellation reason is required');
            }
            const cancellationResult = this.calculateCancellationFee(job.scheduled_at, Number(job.price));
            return await this.prisma.$transaction(async (tx) => {
                await tx.jobs.update({
                    where: { id: jobId },
                    data: {
    ...
    async submitFeedback(userId, jobId, dto) {
        ...
        if (job.customer_id !== customer.id) {
            throw new common_1.ForbiddenException('You can only give feedback for your own jobs');
        }
        if (job.status !== 'paid') {
            throw new common_1.BadRequestException('Can only give feedback for paid jobs');
        }
        if (job.feedbacks.length > 0) {
            throw new common_1.BadRequestException('Feedback already submitted for this job');
        }
        return await this.prisma.$transaction(async (tx) => {
    ...
    async fileDispute(userId, dto) {
        ...
        if (job.customer_id !== customer.id) {
            throw new common_1.ForbiddenException('You can only dispute your own jobs');
        }
        if (!['completed', 'paid'].includes(job.status)) {
            throw new common_1.BadRequestException('Can only dispute completed or paid jobs');
        }
        return await this.prisma.$transaction(async (tx) => {
    ...
    async updateCustomerProfile(userId, dto) {
        ...
        return await this.prisma.$transaction(async (tx) => {
            const userUpdateData = {};
            if (dto.firstName)
                userUpdateData.first_name = dto.firstName;
            if (dto.lastName)
                userUpdateData.last_name = dto.lastName;
            if (dto.phone)
                userUpdateData.phone_number = dto.phone;
            if (Object.keys(userUpdateData).length > 0) {
            ...
            const customerUpdateData = {};
            if (dto.address)
                customerUpdateData.address = dto.address;
            if (dto.zipcode)
                customerUpdateData.zipcode = dto.zipcode;
            if (dto.region)
                customerUpdateData.region = dto.region;
            if (Object.keys(customerUpdateData).length > 0) {
    ...
    async requestNewService(userId, dto) {
        ...
        return {
            message: 'Service request submitted successfully. We will notify you when this service becomes available.',
            requestId: serviceRequest.id,
        };
    }
    calculateCancellationFee(scheduledAt, jobPrice) {
        if (!scheduledAt) {
            return {
                fee: 0,
        ...
        const CANCELLATION_FEE_PERCENTAGE = 0.25;
        const FOUR_DAYS_HOURS = 4 * 24;
        const TWO_DAYS_HOURS = 2 * 24;
