...
let LsmService = class LsmService {
    ...
    async getDashboard(userId) {
        ...
        const [providerStats, jobStats, pendingServiceRequests, disputeStats, recentActivity,] = await Promise.all([
    ...
    async getJobStatsForRegion(lsmId) {
    ...
    async getDisputeStatsForRegion(lsmId) {
    ...
    async getRecentActivityForRegion(lsmId) {
    ...
    async getProviderDetails(userId, providerId) {
        ...
        const completedJobs = provider.jobs.filter((job) => job.status === 'completed' || job.status === 'paid').length;
        const cancelledJobs = provider.jobs.filter((job) => job.status === 'cancelled').length;
        const activeJobs = provider.jobs.filter((job) => ['new', 'in_progress'].includes(job.status)).length;
        const averageRating = provider.feedbacks.length > 0
    ...
    async approveOnboarding(userId, providerId) {
        ...
        if (provider.status !== 'pending') {
            throw new common_1.BadRequestException('Provider is not in pending status');
        }
        const totalDocs = provider.documents.length;
        const verifiedDocs = provider.documents.filter((d) => d.status === 'verified').length;
        if (totalDocs < 2) {
            throw new common_1.BadRequestException('Provider must have at least 2 documents uploaded');
        }
        if (verifiedDocs !== totalDocs) {
            throw new common_1.BadRequestException(`Not all documents are verified. ${verifiedDocs}/${totalDocs} verified.`);
        }
        ...
        await this.prisma.notifications.create({
            data: {
                ...
                message: 'Congratulations! Your account has been approved. You can now start receiving job requests.',
            },
        });
        return {
            id: updated.id,
    ...
    async setProviderStatus(userId, providerId, dto) {
        ...
        if (dto.status === provider.status) {
            throw new common_1.BadRequestException(`Provider is already ${dto.status}`);
        }
        const activeJobs = provider.jobs;
        if (dto.status === 'inactive' && activeJobs.length > 0 && !dto.forceDeactivate) {
            throw new common_1.BadRequestException(`Provider has ${activeJobs.length} active jobs. ` +
    ...
    async requestBan(userId, providerId, dto) {
        ...
        if (existingRequest) {
            throw new common_1.BadRequestException('A ban request for this provider is already pending');
        }
        return await this.prisma.$transaction(async (tx) => {
    ...
    async getDisputes(userId, filters) {
        const lsm = await this.prisma.local_service_managers.findUnique({
            where: { user_id: userId },
        });
        if (!lsm) {
        ...
        return {
            data: disputes.map((dispute) => ({
                ...
                provider: {
                    ...
                    businessName: dispute.job.service_provider.business_name ||
    ...
    async getDisputeDetails(userId, disputeId) {
        ...
        if (dispute.job.service_provider.lsm_id !== lsm.id) {
            throw new common_1.ForbiddenException('This dispute is not in your region');
        }
    ...
    async joinDisputeChat(userId, disputeId) {
        ...
        const chat = dispute.job.chats[0];
        if (!chat) {
            throw new common_1.NotFoundException('Chat not found for this dispute');
        }
        if (!chat.lsm_invited) {
            throw new common_1.BadRequestException('LSM has not been invited to this chat');
        }
        if (chat.lsm_joined) {
            throw new common_1.BadRequestException('LSM has already joined this chat');
        }
    ...
    async getJobsInRegion(userId, filters) {
        const lsm = await this.prisma.local_service_managers.findUnique({
            where: { user_id: userId },
        });
        if (!lsm) {
    ...
    async getServiceRequestsHistory(userId, filters) {
        const lsm = await this.prisma.local_service_managers.findUnique({
            where: { user_id: userId },
        });
        if (!lsm) {
    ...
    async getProviderReviews(lsmUserId, providerId, filters) {
        const lsm = await this.prisma.local_service_managers.findUnique({
        ...
        if (provider.lsm_id !== lsm.id) {
            throw new common_1.ForbiddenException('Provider is not in your region');
        }
    ...
    async getProviderReviewStats(lsmUserId, providerId) {
        const lsm = await this.prisma.local_service_managers.findUnique({
            where: { user_id: lsmUserId },
        });
        if (!lsm) {
